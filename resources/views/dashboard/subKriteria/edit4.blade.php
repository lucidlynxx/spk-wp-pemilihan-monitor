    ...
    <main>
        <div class="container-fluid px-4">
            ...
            <div class="card text-bg-light mb-4">
                ...
                <div class="card-body">
                    <form action="/dashboard/data-sub-kriteria4/{{ $subkriteria->slug }}" method="POST" enctype="multipart/form-data">
                        @method('put')
                        @csrf

                        <div class="mb-3 row">
                            <label for="refreshRate" class="col-sm-2 col-form-label text-end fw-semibold text-secondary">Nama Sub Kriteria</label>
                            <div class="col-sm-10">
                              <input type="text" class="form-control @error('refreshRate')
                                  is-invalid
                              @enderror" id="refreshRate" value="{{ old('refreshRate', $subkriteria->refreshRate) }}" name="refreshRate" required>
                              @error('refreshRate')
                        ...
                        <div class="mb-3 row">
                            <label for="nRefreshRate" class="col-sm-2 col-form-label text-end fw-semibold text-secondary">Nilai</label>
                            <div class="col-sm-10">
                              <input type="number" class="form-control @error('nRefreshRate')
                                  is-invalid
                              @enderror" id="nRefreshRate" value="{{ old('nRefreshRate', $subkriteria->nRefreshRate) }}" name="nRefreshRate" required>
                              @error('nRefreshRate')
                                <div class="invalid-feedback">
                                  {{ $message }}
                                </div>
